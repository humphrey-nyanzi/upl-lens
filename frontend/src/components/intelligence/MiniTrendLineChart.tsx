import { lazy, Suspense } from "react";

import { ChartEmptyState } from "../charts/ChartPrimitives";
import type { IntelligenceTone } from "./ComparisonBars";

export type MiniTrendDatum = {
  key: string;
  label: string;
  value: number;
  secondaryValue?: number | null;
  tone?: Exclude<IntelligenceTone, "navy">;
};

export type MiniTrendLineChartProps = {
  title?: string;
  description?: string;
  data: MiniTrendDatum[];
  valueFormatter?: (value: number) => string;
  emptyLabel?: string;
  height?: "compact" | "regular";
  valueLabel?: string;
};

const MiniTrendLineChartCanvas = lazy(() =>
  import("./MiniTrendLineChartCanvas").then((module) => ({ default: module.MiniTrendLineChartCanvas })),
);

export function MiniTrendLineChart({
  data,
  description,
  emptyLabel = "No trend data available yet.",
  height = "regular",
  title,
  valueFormatter,
  valueLabel = "Value",
}: MiniTrendLineChartProps) {
  const chartData = data.map((item) => ({
    ...item,
    displayValue: valueFormatter ? valueFormatter(item.value) : item.value.toLocaleString(),
  }));

  return (
    <section className={`mini-trend-line-chart mini-trend-line-chart-${height}`} aria-label={title}>
      {title || description ? (
        <div className="intelligence-chart-heading">
          {title ? <h3>{title}</h3> : null}
          {description ? <p>{description}</p> : null}
        </div>
      ) : null}
      {chartData.length ? (
        <>
          <div className="mini-chart-canvas">
            <Suspense fallback={<ChartEmptyState message="Loading chart." />}>
              <MiniTrendLineChartCanvas
                chartData={chartData}
                valueFormatter={valueFormatter}
                valueLabel={valueLabel}
              />
            </Suspense>
          </div>
          <div className="chart-value-list compact" aria-label="Readable chart values">
            {chartData.map((item) => (
              <span key={item.key}>
                <strong>{item.label}</strong>
                {item.displayValue}
              </span>
            ))}
          </div>
        </>
      ) : (
        <ChartEmptyState message={emptyLabel} />
      )}
    </section>
  );
}
